...
class GPT2Classifier:
    def __init__(self, model_name="openai-community/gpt2", num_labels=2, max_length=500):
        """Initialize model Class"""
        start_time = time.time()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = GPT2Tokenizer.from_pretrained(model_name, cache_dir='/scratch/ssd004/scratch/csaleh')
        self.tokenizer.pad_token = self.tokenizer.eos_token
        self.tokenizer.padding_side = "left"
        self.model = GPT2ForSequenceClassification.from_pretrained(model_name, cache_dir='/scratch/ssd004/scratch/csaleh', num_labels=num_labels)
...
if __name__ == "__main__":
    ...
    classifier = GPT2Classifier(num_labels=2, max_length=1024)
