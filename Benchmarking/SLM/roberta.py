...
class RobertaClassifier:
    def __init__(self, model_name="FacebookAI/roberta-base", num_labels=2, max_length=500, dropout_rate=0.2):
        """Initialize model Class"""
        start_time = time.time()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = RobertaTokenizerFast.from_pretrained(model_name, cache_dir='/scratch/ssd004/scratch/csaleh')
        self.model = RobertaForSequenceClassification.from_pretrained(model_name, cache_dir='/scratch/ssd004/scratch/csaleh', num_labels=num_labels)
...
if __name__ == "__main__":
    ...
    classifier = RobertaClassifier(num_labels=2, max_length=512, dropout_rate=0.3)
