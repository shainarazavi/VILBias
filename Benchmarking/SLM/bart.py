...
class BartClassifier:
    def __init__(self, model_name="facebook/bart-base", num_labels=2, max_length=500):
        """Initialize model Class"""
        start_time = time.time()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = BartTokenizer.from_pretrained(model_name, cache_dir='/scratch/ssd004/scratch/csaleh')
        self.model = BartForSequenceClassification.from_pretrained(model_name, cache_dir='/scratch/ssd004/scratch/csaleh', num_labels=num_labels)
...
if __name__ == "__main__":
    ...
    classifier = BartClassifier(num_labels=2, max_length=1024)
