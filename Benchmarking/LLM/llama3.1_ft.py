...
from transformers import PreTrainedTokenizerFast, LlamaForSequenceClassification, AdamW, BitsAndBytesConfig
...
class LlamaClassifier:
    def __init__(self, model_name="meta-llama/Llama-3.1-8B-Instruct", num_labels=2, max_length=500):
        """Initialize model Class"""
        start_time = time.time()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = PreTrainedTokenizerFast.from_pretrained(model_name, cache_dir='/scratch/ssd004/scratch/csaleh')
        ...
        self.model = LlamaForSequenceClassification.from_pretrained(model_name, cache_dir='/scratch/ssd004/scratch/csaleh', quantization_config=bnb_config, num_labels=num_labels)
...
if __name__ == "__main__":
    ...
    print("\nTest Set Evaluation:")
    classifier.eval(test_dataset, batch_size=4)
