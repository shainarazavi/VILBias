...
processor = LlavaNextProcessor.from_pretrained("llava-hf/llava-v1.6-mistral-7b-hf", trust_remote_code=True)
...
def label_and_reason(unique_id, content, date_published):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            print("before inputs")
            inputs = processor(images=compressed_image, text=prompt, return_tensors="pt").to("cuda:0")

            print("before generate")
            generate_ids = model.generate(**inputs, max_new_tokens=1536, temperature=0.2)
