...
print("before calling the processor from pretrained")
processor = AutoProcessor.from_pretrained("/model-weights/pixtral-12b/", trust_remote_code=True)
model = LlavaForConditionalGeneration.from_pretrained(
    "/model-weights/pixtral-12b/",
    device_map="cuda",
    trust_remote_code=True,
    torch_dtype="auto",
    attn_implementation="flash_attention_2",
)
print("after calling the processor from pretrained")
...
def label_and_reason(unique_id, content, date_published):
    ...
    try:
        inputs = processor(full_prompt, compressed_image, return_tensors="pt").to("cuda:0")
        
        with torch.no_grad():
            generate_ids = model.generate(**inputs, max_new_tokens=1536, temperature=0.2, eos_token_id=processor.tokenizer.eos_token_id)
