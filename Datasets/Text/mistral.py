...
tokenizer = AutoTokenizer.from_pretrained("/model-weights/Mistral-7B-Instruct-v0.3/")
model = AutoModelForCausalLM.from_pretrained("/model-weights/Mistral-7B-Instruct-v0.3/", torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2", device_map="cuda")
model_name = 'mistral'
...
def label_and_reason(content):
    ...
    try:
        ...
        with torch.no_grad():
            output = model.generate(**inputs, temperature=0.2, max_new_tokens=350)  
